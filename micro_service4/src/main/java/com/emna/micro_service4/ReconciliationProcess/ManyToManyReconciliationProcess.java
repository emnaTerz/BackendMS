...
public class ManyToManyReconciliationProcess implements ReconciliationProcess {
    ...
    public void process(ReconciliationConfiguration reconciliationConfiguration) throws ScriptException, IOException {
        ...
        for (MatchingResult matchingResult : matchingResults) {
            ...
            Set<String> targetMessageIds = matchingResult.getTargetMessages().values().stream().collect(Collectors.toSet());
            ...
            for (AttributesToReconciliation attributesToReconciliation : attributesToReconciliationList) {
                ...
                for (String sourceId : matchingResult.getSourceMessages().values()) {
                    String cleanedSourceId = sourceId.replaceAll("[^a-zA-Z0-9-]", ""); // Extract clean ID

                    Map<String, String> sourceValues = getValuesForPendingMessage(
                            Collections.singletonMap("sourceMessageId", cleanedSourceId),
                    ...
                    System.out.println("Accumulated source value for sourceId " + cleanedSourceId + ": " + sourceSum);
                }

                // Evaluate target values for all target messages
                for (String targetId : matchingResult.getTargetMessages().values()) {
                    // Cleanly extract the target message ID
                    String cleanedTargetId = targetId.replaceAll("[^a-zA-Z0-9-]", ""); // Remove all except alphanumeric and dashes

                    Map<String, String> targetValues = getValuesForPendingMessage(
                            Collections.singletonMap("targetMessageId", cleanedTargetId),
                    ...
                    System.out.println("Accumulated target value for targetId " + cleanedTargetId + ": " + targetSum);
                    ...
                    if (Math.abs(sourceSum - targetSum) <= Double.parseDouble(reconciliationConfiguration.getTolerance())) {
                        targetMessagesMap.put(cleanedTargetId, "Target Message " + cleanedTargetId);
    ...
    private List<String> cleanAndExtractIds(Map<String, String> messageMap) {
        List<String> cleanedIds = new ArrayList<>();
        for (Map.Entry<String, String> entry : messageMap.entrySet()) {
            String value = entry.getValue();

            // Clean the value by removing any unwanted prefix
            if (value.contains("targetMessageId")) {
                value = value.replace("targetMessageId", "");
            }
            value = value.replace("=", ""); // Remove any '=' character

            // Store the cleaned ID
            cleanedIds.add(value.trim());
        }
    ...
    private Map<String, String> getValuesForPendingMessage(Map<String, String> messageMap, Map<Integer, IndexConfigurationAttributeToAdd> expectedAttributes) {
        ...
        List<String> cleanedIds = cleanAndExtractIds(messageMap);

        for (String pendingMessageId : cleanedIds) {
            System.out.println("Fetching attributes for PendingMessage ID: " + pendingMessageId);

            // Now we correctly fetch attributes using just the cleaned ID
    ...
    private void updateReconciliationStatus(MatchingResult matchingResult, boolean isReconciled) {
        ...
        matchingResult.getTargetMessages().values().forEach(targetMessageId -> {
            if (targetMessageId.startsWith("Target Message ")) {
                targetMessageId = targetMessageId.substring("Target Message ".length());
            }
